...
public abstract class ModItemModelBaseProvider extends ItemModelProvider {
    ...
    public ModItemModelBaseProvider(PackOutput packOutput, ExistingFileHelper existingFileHelper) {
        super(packOutput, Constants.MODID, existingFileHelper);
    }

    protected void trimmedArmorItem(RegistryObject<? extends Item> itemRegistryObject) {
    ...
    protected ItemModelBuilder simpleBlockItem(RegistryObject<Block> item) {
        return withExistingParent(item.getId().getPath(),
                new ResourceLocation("item/generated")).texture("layer0",
                new ResourceLocation(MOD_ID,"item/" + item.getId().getPath()));
    }
